...
class KMeansTests(unittest.TestCase):
    def test_parseSingleDocument(self):
        ...
        kMeans = kmeans.kMeans.KMeans(docModel)

        # act
        highestSentences = kMeans.buildDistances()  # assert
        for sentenceResult in highestSentences:
            print "----------------"
            print str(sentenceResult[0]) + " " + str(sentenceResult[1])
